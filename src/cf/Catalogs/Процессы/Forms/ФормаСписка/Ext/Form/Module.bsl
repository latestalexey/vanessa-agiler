...
&НаКлиенте
Процедура ПоказатьСхему(Команда)
	ТекущиеДанные = Элементы.Список.ТекущиеДанные;
	Если Ложь
		Или ТекущиеДанные = Неопределено
		Или ТекущиеДанные.ЭлементПроцесса <> ПредопределенноеЗначение("Перечисление.ЭлементыПроцесса.Старт")
		Тогда
...
&НаКлиенте
Процедура СформироватьСхему(ТабДок)
