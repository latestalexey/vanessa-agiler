...
Процедура ПередЗаписью(Отказ)
	...
	Если ПустаяСтрока(Заголовок) Тогда
		Заголовок = 
		"Как " + ?(ЗначениеЗаполнено(Персона), СокрЛП(Персона), "X") + 
		" Хочу " + ?(ЗначениеЗаполнено(НеобходимаяФункциональность), СокрЛП(НеобходимаяФункциональность), "Y") + 
		" Чтобы " + ?(ЗначениеЗаполнено(КлючевоеДействие), СокрЛП(КлючевоеДействие), "Z");
	КонецЕсли;
