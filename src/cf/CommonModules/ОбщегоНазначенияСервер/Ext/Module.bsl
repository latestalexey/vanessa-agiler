...
Функция ПолучитьКаталогПроекта(Ссылка) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.БизнесЦенности") Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	БизнесЦенности.Владелец.КаталогПроекта КАК КаталогПроекта
		|ИЗ
		|	Справочник.БизнесЦенности КАК БизнесЦенности
		|ГДЕ
		|	БизнесЦенности.Ссылка = &Ссылка";
...
Функция ПолучитьПутьКFeatureФайлу(ПользовательскаяИстория) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БизнесЦенностиПроекта КАК ЦелиПроекта
...
Процедура ЗаполнитьОпределение(ЭтотОбъект, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	Если Не ПустаяСтрока(ТекстЗаполнения) Тогда
		СтандартнаяОбработка = Ложь;
		
		ЭтотОбъект.Определение = ТекстЗаполнения;
	КонецЕсли;
КонецПроцедуры
